...
class SkillMarketplaceTests(unittest.TestCase):
    ...
    def test_desktop_organizer_server_builds_and_executes_plan(self) -> None:
        with self._runtime_dir("desktop-organizer-server") as runtime_dir:
            ...
            module_path = Path("skills") / "official" / "desktop-organizer" / "mcp" / "server.py"
            spec = importlib.util.spec_from_file_location("desktop_organizer_server", module_path)
    ...
    def test_desktop_organizer_recursive_preview_supports_batch_detail(self) -> None:
        with self._runtime_dir("desktop-organizer-batch-detail") as runtime_dir:
            folder = Path(runtime_dir) / "Desktop"
            folder.mkdir(parents=True, exist_ok=True)
            keep = folder / "keep"
            keep.mkdir()
    ...
    def test_desktop_organizer_recursive_execute_supports_batch_selection(self) -> None:
        with self._runtime_dir("desktop-organizer-execute-batch") as runtime_dir:
            folder = Path(runtime_dir) / "Desktop"
            folder.mkdir(parents=True, exist_ok=True)
            keep = folder / "keep"
            keep.mkdir()
    ...
    def test_desktop_organizer_summary_lists_blocked_files(self) -> None:
        with self._runtime_dir("desktop-organizer-blocked-summary") as runtime_dir:
            folder = Path(runtime_dir) / "Desktop"
            folder.mkdir(parents=True, exist_ok=True)
            keep = folder / "keep"
            keep.mkdir()
    ...
    def test_desktop_organizer_execute_plan_continues_after_filesystem_error(self) -> None:
        with self._runtime_dir("desktop-organizer-rename-error") as runtime_dir:
            folder = Path(runtime_dir) / "Desktop"
            folder.mkdir(parents=True, exist_ok=True)
            (folder / "invoice.pdf").write_text("pdf", encoding="utf-8")
            (folder / "photo.jpg").write_text("jpg", encoding="utf-8")
