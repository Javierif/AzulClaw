...
type StepType = "text" | "textarea" | "identity" | "azure" | "path";
...
const wizardQuestions: WizardQuestion[] = [
  ...
  { id: "IDENTITY", title: "How should AzulClaw work with you?", helper: "Define its main role and the way it should communicate with you. Keep it practical.", placeholder: "", type: "identity", emoji: "Role" },
...
function buildTextAnswers(profile: SetupProfile) {
  return [profile.name, profile.role, [profile.tone, profile.style, profile.autonomy].filter(Boolean).join(", ")];
}
...
function buildProfileFromWizard(
  ...
  const [name, role, temper] = state.answers;
...
export function SetupWizardShell({
  ...
  const draftProfile = useMemo(
    () => buildProfileFromWizard(profile, { answers, azureConfig, workspaceRoot, confirmSensitiveActions }, azureDeployments),
    [answers, azureConfig, azureDeployments, confirmSensitiveActions, profile, workspaceRoot],
  );
  ...
  useEffect(() => {
    function onKey(event: KeyboardEvent) {
      if (event.key !== "Enter" || event.shiftKey || isExiting || isFinalStep || singleStepMode || activeQuestion?.type === "identity") return;
  ...
  }, [activeQuestion?.type, currentStep, isExiting, isFinalStep, singleStepMode]);

  function handleAnswerChange(value: string) {
    setAnswers((current) => current.map((item, index) => (index === currentStep ? value : item)));
  }

  function handleIndexedAnswerChange(index: number, value: string) {
    setAnswers((current) => {
      const next = [...current];
      next[index] = value;
      return next;
  ...
  const shellClass = onboardingRequired
    ? "hw-fullscreen"
    : `hw-contained card${singleStepMode ? " hw-single-step" : ""}`;
  const canSkipStep = activeQuestion?.type === "azure" && !azureConfig.connected;
        ...
        <div className="card panel-stack">
          <div className="panel-heading" style={{ borderBottom: "1px solid var(--line)", paddingBottom: "20px", marginBottom: "4px" }}>
            <div>
              ...
              <p className="hint-text" style={{ margin: "4px 0 0" }}>A compact editor for the parts that shape how AzulClaw works locally.</p>
            </div>
          ...
          <div className="two-column-grid">
            <section className="subcard form-section hw-settings-compact-card">
              ...
              <label className="form-field">
                ...
                <input
                  type="text"
                  value={answers[0] ?? ""}
                  onChange={(e) => handleIndexedAnswerChange(0, e.target.value)}
                />
              </label>
              <label className="form-field">
                <span>Main role</span>
                <textarea
                  rows={4}
                  value={answers[1] ?? ""}
                  onChange={(e) => handleIndexedAnswerChange(1, e.target.value)}
                />
              </label>
              <label className="form-field">
                ...
                <textarea
                  rows={4}
                  value={answers[2] ?? ""}
                  onChange={(e) => handleIndexedAnswerChange(2, e.target.value)}
                />
              </label>
            </section>
            ...
            <section className="subcard form-section hw-settings-compact-card">
              ...
              <p className="hint-text" style={{ margin: 0 }}>
                This folder defines AzulClaw&apos;s local boundary: files stay inside it and memory lives under <code>.azul</code>.
              </p>
              ...
              {previewMemoryDbPath(workspaceRoot) && (
                <div className="workspace-banner">
                  ...
                  <Tooltip text={previewMemoryDbPath(workspaceRoot)} className="workspace-banner-path">
                    {previewMemoryDbPath(workspaceRoot)}
                  </Tooltip>
                </div>
              )}
              ...
              <div style={{ marginTop: "8px" }}>
                <p className="eyebrow" style={{ marginBottom: "10px" }}>Sensitive actions</p>
  ...
  return (
    <div className={shellClass}>
        ...
        {!isFinalStep ? (
          <div key={currentStep} className={`hw-content ${contentAnim}`}>
            ...
            <div className="hw-answer-wrap">
              ...
              {activeQuestion.type === "identity" && (
                <div className="hw-identity-grid">
                  <label className="hw-modal-field hw-identity-card">
                    <span className="hw-field-label">Main role</span>
                    <textarea
                      id="hw-identity-role"
                      className="hw-textarea"
                      value={answers[1] ?? ""}
                      placeholder="Technical companion for coding, decisions and day-to-day execution."
                      onChange={(event) => handleIndexedAnswerChange(1, event.target.value)}
                      autoFocus
                    />
                    <span className="hw-inline-note">Describe the job it should default to when helping you.</span>
                  </label>
                  <label className="hw-modal-field hw-identity-card">
                    <span className="hw-field-label">Behaviour and tone</span>
                    <textarea
                      id="hw-identity-behaviour"
                      className="hw-textarea"
                      value={answers[2] ?? ""}
                      placeholder="Direct, clear, technical and proactive. Confirm sensitive actions."
                      onChange={(event) => handleIndexedAnswerChange(2, event.target.value)}
                    />
                    <span className="hw-inline-note">This feeds tone, style and autonomy defaults.</span>
                  </label>
                </div>
              )}
              ...
              {activeQuestion.type === "path" && (
                <div className="hw-workspace-wrap">
                  <div className="hw-workspace-panel">
                    <p className="hw-inline-note">This folder becomes AzulClaw&apos;s local operating boundary.</p>

                    <div className="hw-workspace-facts">
                      <div className="hw-workspace-fact">
                        <span className="hw-field-label">File sandbox</span>
                        <p className="hw-inline-note">Tools that read, create or move files should stay inside this workspace.</p>
                      </div>
                      <div className="hw-workspace-fact">
                        <span className="hw-field-label">Persistent memory</span>
                        <p className="hw-inline-note">The SQLite memory database is created automatically in a hidden <code>.azul</code> folder here.</p>
                      </div>
                      <div className="hw-workspace-fact">
                        <span className="hw-field-label">Practical rule</span>
                        <p className="hw-inline-note">Use a folder dedicated to AzulClaw, not your whole user profile or desktop root.</p>
                      </div>
                    </div>
            ...
            {!singleStepMode && activeQuestion.type !== "textarea" && activeQuestion.type !== "identity" && (
              <p className="hw-hint">
                <span>{nextHint}</span>
      ...
      <footer className="hw-footer">
        {singleStepMode ? (
          <>
            <span className="hw-step-label">Focused editor</span>
            {saveError && <span className="hw-inline-note hw-inline-note-warning">{saveError}</span>}
            <div style={{ display: "flex", gap: "10px" }}>
              <button type="button" className="hw-btn-primary" onClick={() => void handleSave(profile.is_hatched || !onboardingRequired)} disabled={isSaving}>
                {isSaving ? "Saving..." : "Apply changes"}
              </button>
            </div>
          </>
          ...
          <>
            <button type="button" className="hw-btn-ghost" onClick={handleBack} disabled={currentStep === 0}>Back</button>
            ...
            {isFinalStep ? (
              ...
              <button type="button" className="hw-btn-primary" onClick={handleNext} >{nextButtonLabel}</button>
            )}
          </>
        )}
      </footer>
