...
- The current deployment works best with sessions enabled on `bot-outbound`.
...
3. The `POST /api/messages` endpoint returns `200`.
...
- `bot-outbound` exists
- session configuration is consistent
